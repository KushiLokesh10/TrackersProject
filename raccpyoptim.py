...
while not caught:
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            caught = True  # Exit the loop but keep the screen open
            break  # Exit the event loop to prevent closing the window

    pygame.display.flip()

# Outside the main loop
# ... (assuming cop_thread and thief_thread are defined)
cop_thread.join()
thief_thread.join()
