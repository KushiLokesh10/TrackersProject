...
obstacles = [(300, 200, 20 , 200), (400, 200, 30 , 200), (200, 500, 120, 10), (500, 300, 70, 70) , (0, 0, WIDTH, 10), (0, 0, 10, HEIGHT), (0, HEIGHT - 10, WIDTH, 10), (WIDTH - 10, 0, 10, HEIGHT)]
...
def move_cop():
    ...
    while True:
        ...
        if distance > 20:
            ...
            next_x = cop_pos[0] + int(math.cos(direction) * 5)
            next_y = cop_pos[1] + int(math.sin(direction) * 5)
            ...
            if not any(rect.collidepoint(next_x, next_y) for rect in obstacle_rects):
                ...
                dy = thief_pos[1] - cop_pos[1]
                direction = math.atan2(dy, dx)
                cop_pos[0] += int(math.cos(direction + math.pi / 2) * 5)
                cop_pos[1] += int(math.sin(direction + math.pi / 2) * 5)
        if distance <= 20:  # Check if the cop catches the thief
            ...
            thief_pos = [random.randint(0, WIDTH), random.randint(0, HEIGHT)]  # Reset thief position
            print("Thief reset to position:", thief_pos)  # Print the new position of the thief
